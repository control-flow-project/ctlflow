...
`identityd` owns Users, identity links, provider configuration, login
transactions, Sessions, and invocation identity. `authd` cannot create,
modify, infer, or cache those records as an independent authority.
...
- `authd` is stateless apart from bounded in-flight protocol state.
