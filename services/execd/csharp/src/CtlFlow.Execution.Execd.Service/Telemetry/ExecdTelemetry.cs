...
internal sealed class ExecdTelemetry : IDisposable
{
    internal const string SourceName = "ctlflow.execd";
    ...
    private static readonly Action<ILogger, string, string, double, Exception?>
        LogOperationCompletion = LoggerMessage.Define<string, string, double>(
            ...
            new EventId(1, "ExecdOperationCompleted"),
    ...
    private readonly ILogger<ExecdTelemetry> _logger;
    private readonly Counter<long> _requests;
    private readonly Histogram<double> _duration;
    private readonly TracerProvider _traces;
    private readonly MeterProvider _metrics;

    internal ExecdTelemetry(
        TelemetrySettings settings,
        ILogger<ExecdTelemetry> logger)
    {
        _logger = logger;
        _requests = _meter.CreateCounter<long>(
            "ctlflow.execd.requests",
            description: "Completed execd operations");
        _duration = _meter.CreateHistogram<double>(
            "ctlflow.execd.duration",
            unit: "ms",
            description: "execd operation duration");

        _traces = Sdk.CreateTracerProviderBuilder()
            .SetResourceBuilder(CreateResource())
            .SetSampler(new AlwaysOnSampler())
            .AddSource(SourceName)
            .AddSource(CtlFlow.Execution.Execd.Db.ExecutionDbTelemetry.SourceName)
        ...
        Metadata headers)
    {
        ...
        activity?.SetTag("rpc.service", "ctlflow.execution.v1.ExecutionService");
    ...
    internal Activity? StartDependencyCall(
        string service,
        string operation)
    {
        var activity = _activitySource.StartActivity(
    ...
    internal void RecordDependencyCall(
        Activity? activity,
        string operation,
        string outcome,
        long startedTimestamp) =>
    ...
    internal static ResourceBuilder CreateResource() =>
        ResourceBuilder.CreateEmpty()
            .AddService(
                ...
                serviceVersion: typeof(ExecdTelemetry).Assembly
                    .GetName()
                    .Version?
                    .ToString() ?? "0.0.0");
