...
const databasePath = path.join(directory, "execd.sqlite");
const modelAuditProject = path.join(
  testsRoot,
  "CtlFlow.Execution.Execd.IntegrationTests",
  "CtlFlow.Execution.Execd.IntegrationTests.csproj");
...
const modelAuditPublication = await publishCSharpService({
  ...
  executableName: "CtlFlow.Execution.Execd.IntegrationTests"
});
...
const servicePublication = await publishCSharpService({
  ...
  executableName: "CtlFlow.Execution.Execd.Service"
});
...
process.stdout.write("execd rejected an unimplemented database provider\n");
