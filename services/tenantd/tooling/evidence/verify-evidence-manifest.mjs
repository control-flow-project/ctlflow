...
const manifestService = "ctlflow.tenancy.v1.TenantService";
...
const allowedStatuses = new Set(documentedStatuses);
...
assertSameValues(
  "RPC evidence",
  [...operations.keys()],
  await readRpcMethods());
assertSameValues(
  "documented gRPC status evidence",
  [...statuses],
  documentedStatuses);
await verifyReferences(manifest.crossCutting);
process.stdout.write("tenantd evidence manifest verified\n");
...
function parseManifest(value) {
  ...
  if (value.schemaVersion !== 1
    ...
    throw new Error("Tenantd evidence manifest shape is invalid");
  }
  verifyReleaseGates(value.releaseGates, "tenantd", true);
  return {
