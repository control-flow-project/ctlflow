...
export async function createTenantdDatabase(
  storage: TestKubernetesStorage
): Promise<TenantdTestDatabase> {
  const root = path.join(storage.hostRoot, "tenantd", "databases");
  await mkdir(root, { recursive: true });
  const directory = await mkdtemp(path.join(root, "database-"));
