        ...
        CancellationToken cancellation)
    {
        ...
        if (rows.Count == 0)
        {
            throw new InvalidOperationException(
                "Audit outbox lease is no longer claimable");
        }
        ...
        foreach (var row in rows)
        {
            ...
            entry.DeliveryState = 3;
            entry.Revision++;
            entry.LeaseId = null;
            entry.LeaseExpiresAtUnixMilliseconds = null;
            entry.FailureCode = (int)failureCode;
        }
