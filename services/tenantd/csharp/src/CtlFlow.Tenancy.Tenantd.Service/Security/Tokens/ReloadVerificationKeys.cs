...
internal static partial class JsonWebKeys
{
    ...
    internal static async Task<VerificationKeySnapshot> ReloadVerificationKeys(
        string path,
        ...
        JsonElement root)
    {
        ...
        foreach (var key in keyArray.EnumerateArray())
        {
            ...
            if (keyId.Length is < 1 or > 128
                || !keys.TryAdd(
                    keyId,
                    new RsaVerificationKey(
                        DecodeBase64Url(ReadString(key, "n")),
                        DecodeBase64Url(ReadString(key, "e")))))
            {
    ...
    private static byte[] DecodeBase64Url(string value)
    {
        ...
        {
            throw new InvalidDataException(
                "The verification-key set contains invalid key material",
                exception);
        }
    }
}
