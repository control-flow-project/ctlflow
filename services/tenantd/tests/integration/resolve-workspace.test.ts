...
test("exports bounded ResolveWorkspace traces, metrics, and logs without request data", async () => {
  ...
  await waitForTelemetry(
    requireCollector().logsPath,
    (value) => value.includes("ResolveWorkspace completed"));
  ...
  const exported = await readExportedTelemetry();
  for (const value of [
    requireKubernetes().callerToken,
    invocationToken,
    workspace.tenantId,
...
test("correlates structured logs with the request trace", async () => {
  ...
  await waitForTelemetry(
    requireCollector().logsPath,
    (value) =>
      hasCorrelatedLog(value, traceId, "ResolveWorkspace completed"));
});
...
test("nests the DB span under the ResolveWorkspace server span in one trace", async () => {
  ...
  await waitForTelemetry(
    requireCollector().tracesPath,
    (value) => {
      const spans = spansForTrace(value, traceId);
...
function collectLogRecords(batch: unknown): { traceId?: unknown }[] {
  const records: { traceId?: unknown }[] = [];
  ...
  for (const resource of resourceLogs) {
    ...
    for (const scope of scopeLogs) {
      ...
      for (const record of logRecords) {
        records.push(record as { traceId?: unknown });
      }
    }
  }
...
function hasCorrelatedLog(
  ...
  for (const line of content.split("\n")) {
    ...
    for (const record of collectLogRecords(batch)) {
      if (record.traceId === traceId
        && JSON.stringify(record).includes(bodySubstring)) {
        return true;
      }
    }
  }
...
function spansForTrace(content: string, traceId: string): OtlpSpan[] {
  ...
  for (const line of content.split("\n")) {
    ...
    for (const span of collectSpans(batch)) {
...
async function readExportedTelemetry(): Promise<string> {
  return (await Promise.all([
    readFile(requireCollector().tracesPath, "utf8"),
    readFile(requireCollector().metricsPath, "utf8"),
    readFile(requireCollector().logsPath, "utf8")
  ])).join("\n");
}
