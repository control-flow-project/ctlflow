...
assertSame(
  [...usedTests].sort(),
  (await readAllTestTitles()).sort(),
  "canonical test evidence inventory");
process.stdout.write("egressd evidence manifest verified\n");
...
function parseManifest(value) {
  ...
  assert(
    value.schemaVersion === 1
      && value.service === "egressd.http.v1"
      && Array.isArray(value.operations)
      && value.operations.length === 7
    ...
    "Egressd evidence manifest shape is invalid");
  verifyReleaseGates(value.releaseGates, "egressd", false);
  return {
...
async function verifyReferences(references) {
  ...
  for (const reference of references) {
    ...
    assert(!seen.has(identity), `Duplicate test evidence: ${identity}`);
    seen.add(identity);
    ...
    assert(
      ...
      `Evidence test does not exist: ${reference.file} :: `
        + reference.title);
    usedTests.add(identity);
  }
}

async function readAllTestTitles() {
  const files = [
    "api.test.ts",
  ...
  for (const file of files) {
    ...
    for (const title of await readTestTitles(relativePath)) {
      titles.push(`${relativePath}\0${title}`);
    }
  }
  return titles;
}
...
async function readTestTitles(relativePath) {
  const source = await readFile(path.join(serviceRoot, relativePath), "utf8");
