...
const integrationRoot = path.join(serviceRoot, "tests/integration");
...
for (const operation of manifest.operations) {
  ...
  assertSameValues(
    `${operation.name} status evidence`,
    Object.keys(operation.results),
    expectedStatuses);
  for (const references of Object.values(operation.results)) {
...
await verifyCompleteTestInventory();
process.stdout.write("auditd evidence manifest verified\n");
...
function parseReference(value) {
  ...
  if (!/^tests\/integration\/[^/]+\.test\.ts$/u.test(value.file)
      || value.file.endsWith("/auditd.test.ts")) {
...
async function readRpcMethods() {
  const source = await readFile(
    path.join(serviceRoot, "api/proto/v1/auditd.proto"),
    "utf8");
  const root = protobuf.parse(source).root;
  return Object.keys(root.lookupService(manifestService).methods);
}
...
async function verifyReferences(references) {
  ...
  for (const reference of references) {
    ...
    referencedTests.add(identity);
  }
}

async function verifyCompleteTestInventory() {
  const files = (await readdir(integrationRoot))
    .filter((name) =>
      name.endsWith(".test.ts") && name !== "auditd.test.ts")
    .sort();
  const tests = [];
  for (const name of files) {
    const relativePath = `tests/integration/${name}`;
    for (const title of await readTestTitles(relativePath)) {
      tests.push(`${relativePath}\0${title}`);
    }
  }
  assertSameValues(
