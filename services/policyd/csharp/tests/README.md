...
Canonical wire behavior belongs in `services/policyd/tests/` and runs against
the shipping NativeAOT process.

`CtlFlow.Policy.Policyd.IntegrationTests` is the implementation-specific
...
column, key, relationship, index, nullability, type-affinity, and
concurrency-token inventory.

`run-model-audit.mjs` publishes that executable through the same gated,
