...
export async function createInvocationAuthority(
  ...
  return {
    ...
    verificationKey: createVerificationKey(keyId, publicJwk),
    sign: (options = {}) =>
      signInvocationToken(keyId, keys.privateKey, options),
    signPayload: (payloadJson) =>
      signPayload(keyId, keys.privateKey, payloadJson),
...
function signInvocationToken(
  ...
  return signPayload(keyId, privateKey, JSON.stringify(payload));
}
