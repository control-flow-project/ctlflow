...
const manifestService = "ctlflow.packages.v1.PackageService";
...
const documentedStatuses = [...allowedStatuses];
...
assertSameValues("RPC evidence", [...operations.keys()], methods);
assertSameValues(
  "documented gRPC status evidence",
  [...statuses],
  documentedStatuses);
await verifyReferences(manifest.crossCutting);
process.stdout.write("pkgd evidence manifest verified\n");
...
function parseManifest(value) {
  ...
  if (value.schemaVersion !== 1
    ...
    throw new Error("Pkgd evidence manifest shape is invalid");
  }
  verifyReleaseGates(value.releaseGates, "pkgd", true);
...
async function readRpcMethods() {
  const source = await readFile(
    path.join(serviceRoot, "api/proto/v1/pkgd.proto"),
    "utf8");
