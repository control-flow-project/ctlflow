...
namespace CtlFlow.Packages.Pkgd.Service.Telemetry;

internal sealed class PkgdTelemetry : IDisposable
{
    internal const string SourceName = "ctlflow.pkgd";
    ...
    private static readonly Action<ILogger, string, string, double, Exception?>
        LogOperationCompletion = LoggerMessage.Define<string, string, double>(
            ...
            new EventId(1, "PkgdOperationCompleted"),
    ...
    private readonly ILogger<PkgdTelemetry> _logger;
    private readonly Counter<long> _requests;
    private readonly Histogram<double> _duration;
    private readonly TracerProvider _traces;
    private readonly MeterProvider _metrics;

    internal PkgdTelemetry(
        TelemetrySettings settings,
        ILogger<PkgdTelemetry> logger)
    {
        _logger = logger;
        _requests = _meter.CreateCounter<long>(
            "ctlflow.pkgd.requests",
            description: "Completed pkgd operations");
        _duration = _meter.CreateHistogram<double>(
            "ctlflow.pkgd.duration",
            unit: "ms",
            description: "pkgd operation duration");

        _traces = Sdk.CreateTracerProviderBuilder()
            .SetResourceBuilder(CreateResource())
            .SetSampler(new AlwaysOnSampler())
            .AddSource(SourceName)
            .AddSource(CtlFlow.Packages.Pkgd.Db.PackageDbTelemetry.SourceName)
        ...
        Metadata headers)
    {
        ...
        activity?.SetTag("rpc.service", "ctlflow.packages.v1.PackageService");
    ...
    internal static ResourceBuilder CreateResource() =>
        ResourceBuilder.CreateEmpty()
            .AddService(
                ...
                serviceVersion: typeof(PkgdTelemetry).Assembly
                    .GetName()
                    .Version?
                    .ToString() ?? "0.0.0");
