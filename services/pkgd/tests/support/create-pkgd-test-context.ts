...
import {
  getPkgdTestSuite
} from "../suite/get-pkgd-test-suite.js";
import {
  createTestDatabase
} from "./create-test-database.js";
import {
  preparePkgdContextFiles,
  type PkgdContextFiles
} from "./prepare-pkgd-context-files.js";
import {
  type PkgdRunningService
...
export interface PkgdTestContext {
  ...
  readonly service: PkgdRunningService;
  readonly client: PackageServiceClient;
  readonly workloadClient: PackageServiceClient;
  readonly unadmittedOperatorClient: PackageServiceClient;
...
export async function createPkgdTestContext():
Promise<PkgdTestContext> {
  const suite = getPkgdTestSuite();
  let database: TestDatabase | undefined;
  let auditd: AuditdProductionSource | undefined;
  let identityd: IdentitydProductionSource | undefined;
  let service: PkgdRunningService | undefined;
  const clients: PackageServiceClient[] = [];
  ...
  try {
    ...
    const files = await preparePkgdContextFiles({
    ...
    const client = new PackageServiceClient(
      endpoint,
      ...
      options);
    const workloadClient = new PackageServiceClient(
      endpoint,
...
async function stopResources(
  service: PkgdRunningService | undefined,
