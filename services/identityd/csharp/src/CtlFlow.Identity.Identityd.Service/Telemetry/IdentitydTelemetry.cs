...
internal sealed class IdentitydTelemetry : IDisposable
{
    internal const string SourceName = "ctlflow.identityd";
    ...
    private static readonly Action<ILogger, string, string, double, Exception?>
        LogOperationCompletion = LoggerMessage.Define<string, string, double>(
            ...
            new EventId(1, "IdentitydOperationCompleted"),
    ...
    private readonly ILogger<IdentitydTelemetry> _logger;
    private readonly Counter<long> _requests;
    private readonly Histogram<double> _duration;
    private readonly TracerProvider _traces;
    private readonly MeterProvider _metrics;

    internal IdentitydTelemetry(
        TelemetrySettings settings,
        ILogger<IdentitydTelemetry> logger)
    {
        _logger = logger;
        _requests = _meter.CreateCounter<long>(
            "ctlflow.identityd.requests",
            description: "Completed identityd operations");
        _duration = _meter.CreateHistogram<double>(
            "ctlflow.identityd.duration",
            unit: "ms",
            description: "identityd operation duration");

        _traces = Sdk.CreateTracerProviderBuilder()
            .SetResourceBuilder(CreateResource())
            .SetSampler(new AlwaysOnSampler())
            .AddSource(SourceName)
            .AddSource(CtlFlow.Identity.Identityd.Db.IdentityDbTelemetry.SourceName)
        ...
        Metadata headers)
    {
        ...
        activity?.SetTag("rpc.service", "ctlflow.identity.v1.IdentityService");
    ...
    internal static ResourceBuilder CreateResource() =>
        ResourceBuilder.CreateEmpty()
            .AddService(
                serviceName: "identityd",
                serviceNamespace: "ctlflow",
                serviceVersion: typeof(IdentitydTelemetry).Assembly
                    .GetName()
                    .Version?
                    .ToString() ?? "0.0.0");
