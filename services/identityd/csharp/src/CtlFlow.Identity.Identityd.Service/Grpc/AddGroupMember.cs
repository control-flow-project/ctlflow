...
{
    public override async Task<CtlFlow.Identity.V1.GroupMember>
        AddGroupMember(
            ...
            ServerCallContext context)
    {
        ...
        var audit = await CreateAuditContext(
            ...
            now,
            context.CancellationToken);
        var result = await IdentityGroups.AddGroupMember(
            _identityDatabase,
            groupId,
            principalId,
            target,
        ...
        await RecordAdministrationAudit(
            result.AuditIntent,
            context.CancellationToken);
        return CreateGroupMemberMessage(result.Value.Member);
    }
}
