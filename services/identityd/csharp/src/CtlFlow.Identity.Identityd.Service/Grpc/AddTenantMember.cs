...
{
    public override async Task<TenantMember> AddTenantMember(
        ...
        ServerCallContext context)
    {
        ...
        var audit = await CreateAuditContext(
            ...
            now,
            context.CancellationToken);
        var result = await IdentityMemberships.AddTenantMember(
            _identityDatabase,
            accountId,
            tenantId,
            audit,
        ...
        await RecordAdministrationAudit(
            result.AuditIntent,
            context.CancellationToken);
        return CreateTenantMemberMessage(result.Value);
    }
}
